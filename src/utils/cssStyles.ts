...
type BackgroundImageProps = BackgroundGradientProps & {
  url?: string
}
...
export default function cssStyles(theme?: Theme) {
  return {
    ...
    bgImage: (props: BackgroundImageProps) => {
      ...
      const direction = getDirection(props?.direction)
      const startColor =
        props?.startColor || alpha(theme?.palette.background.paper || '#000000', 0.88)
      const endColor = props?.endColor || alpha(theme?.palette.background.paper || '#000000', 0.88)
